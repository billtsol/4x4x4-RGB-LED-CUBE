...
#define latch_pin 2 //  latch pin number used to control the shift registers in the LED cube project
#define blank_pin 4 // this refers to a pin on a microcontroller that is used to control a blanking or disabling function
...
void LED(int level, int row, int column, byte red, byte green, byte blue) {
  ...
  int whichbyte = int(((level * 16) + (row * 4) + column) / 8); // position on the LED matrix
